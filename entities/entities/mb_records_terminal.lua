...
ENT.PrintName = "Records Terminal"
...
if SERVER then
	function ENT:Initialize()
		...
		local model = cfg.JailRecordsTerminalModel or "models/props_lab/monitor02.mdl"
		if not util.IsValidModel(model) then model = cfg.JailTerminalFallbackModel or "models/props_lab/monitor01b.mdl" end
	...
	function ENT:Use(activator)
		...
		if MilBase and MilBase.DatapadOpen then
			MilBase.DatapadOpen(activator, "criminal_records")
		elseif MilBase and MilBase.JailOpenRecords then
			MilBase.JailOpenRecords(activator)
		else
			MilBase.Notify(activator, "Datapad / jail module is not loaded.")
		end
	end
...
function ENT:Draw()
	...
	local name = self.MBJailName or "RECORDS TERMINAL"
	...
	cam.Start3D2D(pos, ang, 0.075)
		...
		draw.SimpleText("PRESS E: OPEN DATAPAD", fontTiny, 0, 12, dim, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
